...
def generate_launch_description():
    ...
    camera_param_uri   = LaunchConfiguration('camera_param_uri',  default = 'package://f1tenth_detection_cpp/config/camera')
    sync_nn            = LaunchConfiguration('sync_nn',           default = True)
    subpixel           = LaunchConfiguration('subpixel',          default = True)

    nnPath             = LaunchConfiguration('nnPath', default = "/home/merline/ws_yolov5/src/f1tenth_detection_cpp/resources/f1tenth_yolov5_416.blob")
    confidence_thresh  = LaunchConfiguration('confidence_thresh',        default = 200)
    ...
    declare_nnPath_cmd = DeclareLaunchArgument(
        'nnPath',
        default_value=nnPath,
        description='Path to the object detection blob needed for detection')
    
    declare_confidence_thresh_cmd = DeclareLaunchArgument(
        'confidence',
        default_value=confidence_thresh,
    ...
    speed_detection_node = launch_ros.actions.Node(
        package='f1tenth_detection_cpp', executable='speed_detection_node',
        output='screen',
   ...
   #)
    ...
    ld.add_action(declare_nnPath_cmd)
    ld.add_action(declare_confidence_thresh_cmd)
    ...
    ld.add_action(speed_detection_node)
